...
import { useLazyGetCaptchaUrlQuery } from "@/features/auth/api/securityApi.ts"
    ...
    login(data).then((res) => {
      ...
      if (res.data?.resultCode === ResultCode.CaptchaError) {
        ...
        getCaptchaUrl().then((res) => {
          if (res.data?.url) {
            dispatch(setCaptchaUrlAC({ captchaUrl: res.data?.url }))
          }
        })
      }
    })
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <FormControl>
          ...
          <FormGroup>
            ...
            />
            {captchaUrl && <img src={captchaUrl} alt="captcha" width="" height="" loading="lazy" />}
            {captchaUrl && <TextField margin="normal" label="Enter text from image" {...register("captcha")} />}
